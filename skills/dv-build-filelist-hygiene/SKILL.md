...
the error text is usually the least useful part of it. The compiler reports the *consequence* of a
missing file, a missing include path, a missing define or a wrong file order — and reports it twenty
times — while the elaborator reports a module it cannot find without ever saying which filelist was
supposed to contain it. The answer almost always lives in the filelists, which nobody audits because
auditing them is tedious rather than hard.
...
directive name sends the engineer to audit a file set that is not the one that failed, which is
...
4. Scope every **Glob** and **Grep** to one directory that actually appears in the flat list. A
   recursive search from the repository root returns tens of thousands of paths and no useful answer.
...
different causes and often different owners:
...
Then **Grep** for the marker that ends compilation. If it is absent, the build never reached
elaboration and every elaboration-shaped theory is wrong.
...
flattened filelist** from step 4. That file, at that line, is the one to explain. Everything
downstream of an unknown type or an unreadable include is noise.
...
| identifier not declared | missing wildcard import, or a genuine typo | Grep the identifier inside the package files only |
...
Record the classification and the one piece of evidence. A classification with no cited line is a
...
include path, define, library search directory, tool switch, comment). Apply the relative-path
resolution rule from the slot table: a nested filelist resolving against its own directory behaves
differently from one resolving against the directory the build started in, and that single difference
explains a surprising number of failures.
...
- **Duplicate module.** For each module named in the failing diagnostics, **Grep** for its
  declaration across the flat-list directories. Two hits in two different files is the real hazard;
  report both paths and both flat-list positions.
...
- **Unreachable.** Entries under a directory that no longer exists usually mean an old release path
  survived a bump. Say which release the surviving paths point at.
...
- **Missing defines.** **Grep** the sources for conditional-compilation directives and collect the
  macro names they test. Compare against the macros set in the filelists. A macro that is tested but
  never set anywhere is a strong candidate cause.
...
- An interface whose type is referenced inside a package must be analysed **before** that package.
  This is the ordering people most often have backwards, and its symptom is an unknown type inside a
  package rather than anything that mentions the interface.
...
1. Different macro definitions — a block filelist that sets a stub or simulation-only macro that the
   top build does not.
2. Different include-path order, so a different copy of the same header wins.
3. Different grouping of files into compilation units, so a macro or a default net type that leaked
   usefully at block level no longer reaches its consumer.
4. A second copy of a module arriving at top through a neighbouring IP filelist.
5. A stub used at block level replaced by real logic at top, exposing a port or parameter mismatch
...
If this failure has to be handed to someone else or matched against an existing one, derive the
signature using `_shared/failure-signature-schema.md` — `phase` is `compile` or `elab` and `kind` is
usually `tool`. Do not restate those rules here.
...
- **One unknown type produces twenty diagnostics.** A single missing package import makes every
  declaration that uses its types fail. The number of errors tells you nothing about the number of
  problems, and fixing the loudest one changes nothing.
- **Macros do not cross file boundaries reliably.** A macro definition is visible only within its
  compilation unit and only from its definition onward. A block build that analyses everything as one
  unit hides this; a top build that analyses per-file exposes it as an undefined macro in a file that
  has not changed in a year. Guarded headers that each consumer includes are the durable answer.
- **A changed default net type leaks forward.** A file that turns implicit nets off and never
  restores them hands that setting to the next file in the same compilation unit. The diagnostic
  lands on an innocent file, which is why it survives so long.
- **A missing timescale directive is an ordering bug, not a missing line.** A module without one
  inherits from whatever was analysed before it, so reordering the filelist can change time
  resolution with no source edit at all.
...
- **Library search directories make a missing file look present.** If the flow searches directories
  ...
  diagnostic whatsoever. Absence of an error is not evidence that the intended file was used.
...
- **A filelist entry missing from disk may be generated.** Check the generated-sources slot before
  calling it stale. Removing a generated entry is how a working flow gets quietly broken for everyone.
...
- the named first diagnostic is the earliest by dependency, not the last one printed and not the one
  with the most alarming wording.
- a proposed addition to a filelist was checked for the file being **already present at a different
  position** — adding it again creates a duplicate-definition hazard that surfaces weeks later.
- nothing generated has been reported as stale.
- the compile-versus-elaboration verdict matches the shape of the diagnostic — a module-not-found
  with no source line is not a syntax problem.
