"""Batch orchestrator: mint review-batch contracts from a validated scoreboard snapshot.
    ...
    semiskill review-issue --snapshot reports/scoreboard.json --phase review \
...
Coordinator-only in practice: issuing a real (non-`_test`) batch requires the
`SEMISKILL_REVIEW_COORDINATOR_DATABASE_URL` identity (`PostgresArtifactStore.
review_coordinator_authentication_context`), which only the review coordinator's dedicated
Postgres login can satisfy — see ADR-029 / J-010f1 for the local development provisioning of that
identity.
"""
...
from semiskill.authoring.snapshot import SnapshotUnavailable, full_input_tree_sha256
